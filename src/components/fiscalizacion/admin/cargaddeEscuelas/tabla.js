...
const Estracto = () => {
    ...
    const getTodos = async () => {
        ...
        const tod = await servicioFisca.listaExtractosescuelas()
        console.log(tod)
        setTodos(tod)
    }
    ...
    const getClients = async () => {
        setActivo(true)
        const datos = await servicioFisca.VerExtractoescuelas(fecha)
        setDats(datos)
        setLoading(false);
    }
    ...
    const cargarcantidades = async (id) => {
        console.log(fecha)
      const datos = await servicioFisca.cargarcantidades(fecha)
      alert(datos)
      
    }
    const cargar = async (id) => {
        console.log(fecha)
      const datos = await servicioFisca.cargarinscripcionesescuelas(fecha)
      alert(datos)
                ...
                {loading ? (<CargaDeTabla />)
                    : (
                        <div>
                            ...
                            <Button
                                onClick={() => cargarcantidades(fecha.id)}
                            >   Cargar Cantidades</Button>
                            <MUIDataTable
