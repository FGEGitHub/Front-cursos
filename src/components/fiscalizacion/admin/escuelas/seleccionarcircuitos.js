...
export default function Clasenueva(props) {
    ...
    const traer = async () => {
        ...
        const turnos = await servicioFide.traercircuitos()
        setEscuelas(turnos)
    ...
    const handleDeterminar = async (event) => {
        ...
        const respuesta = await servicioInscripciones.asignarcircuitos({id:props.id, inscrip:selectedOptions})
        alert(respuesta)
    ...
    return (
        <div>
            ...
            < Tooltip title="Asignar mas llamados">
                < Button onClick={handleClickOpen} >Seleccionar circuitos </Button>
            </Tooltip>
                               ...
                               {props.id}
                                {escuelas ? <>
                                    {escuelas.map((row) => (
                                        ...
                                        <>
                                            <label>
                                                <input type="checkbox" checked={selectedOptions.includes( row.circuito )} onChange={() => handleOptionChange( row.circuito )} />
                                                {row.circuito} 
                                            </label>
                                            <br />
                                        </>
